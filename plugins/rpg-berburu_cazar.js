 ...
 *🐊 ${ar6} ${anti6}*		    *🐓 ${ar12} ${anti12}*`.trim();
  ...
  if (new Date - global.db.data.users[m.sender].lastberburu < 2700000) return conn.reply(m.chat, `Mohon istirahat sejenak untuk melanjutkan berburu\n\n⫹⫺ Waktu ${clockString(time - new Date())}\n${wm}`, m);
...
[null, null]], null)*/
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['TETAPKAN TUJUAN 🎯', 'Umpan Saat Bergerak 🍫 🍇 🍖', 'HEWAN TERDETEKSI!! 🐂 🐅 🐘 🐼', 'HEWAN TERDETEKSI!! 🐖 🐃 🐮 🐒'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 18000);
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['Senjata siap untuk Berburu!!', 'Menguji Senjata 🔫 💣 🪓 🏹', 'MOBIL UNTUK BERBURU!! 🚗 🏍️ 🚜', 'CUACA YANG BAIK UNTUK BERBURU 🧤'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 15000);
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['Mencari alat berburu...', 'Mempersiapkan segalanya untuk berburu!!', 'Mendirikan Tempat Berburu...', 'MENYIAPKAN TEMPAT BERBURU!!'].getRandom()}*`, m, m.mentionedJid ? {mentions: [m.sender]} : {});
  }, 0);
